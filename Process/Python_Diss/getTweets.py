...
authenticate("localhost:7474", "neo4j", "MH4j")
...
result = graph.cypher.execute("MATCH (u:SEEDUSER)-[:TWEETED]-(s:SEEDTWEET) "
                     "WITH u, COUNT(DISTINCT s) AS n_Tweets "
                     "LIMIT 100 "
                     "MATCH (u)-[:TWEETED]-(s2:SEEDTWEET) "
                    "RETURN s2.ID, s2.Text LIMIT 100 "
                     )

breakdowns = {}
for record in result:
    record_str = record['s2.Text'].encode('utf-8')
    #print("%s" % record_str)
    vs = analyzer.polarity_scores(record_str)
    breakdowns[record['s2.ID']] = {'Overlap Zaydman 1':num_words_in_string(wordlistZaydmanOrig,record_str),
...
for key in breakdowns.keys():
    params = {'key':key,'props':breakdowns[key]}
    query = "MATCH (s:STATUS WHERE s.ID = {key} SET s += {props} RETURN s"
    #print(query.format(params))
    tx.append(query,parameters=params)
    tx.process()
#    graph.data(("MATCH (s:STATUS {{ID: {} }}) "
#                "SET s += {params} "
